...
INNER JOIN vp_user
ON vs_nombramiento.id_funcionario = vp_user.user_id  where vs_nombramiento.status >= 1 ORDER BY fecha_inicio DESC";
    $p = $pdo->prepare($sql);
    $p->execute();
