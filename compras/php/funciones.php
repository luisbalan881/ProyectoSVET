...
//$sql = "SELECT da_solicitud_compra.id_solicitud_compra, vp_deptos.dep_nm, CONCAT(vp_user.user_nm1, ' ' , vp_user.user_ap1) AS solicitante, da_solicitud_compra.fecha_creacion, da_solicitud_compra.clasificacion, da_solicitud_compra.estado_solicitud, da_solicitud_compra.id_solicitante FROM `da_solicitud_compra`
//on da_solicitud_compra.id_dep=vp_deptos.dep_id order by da_solicitud_compra.id_solicitud_compra desc";
$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
$sql = "SELECT da_solicitud_compra.id_solicitud_compra, vp_deptos.dep_nm, CONCAT(vp_user.user_nm1, ' ' , vp_user.user_ap1) AS solicitante, da_solicitud_compra.fecha_creacion, da_solicitud_compra.clasificacion, da_solicitud_compra.estado_solicitud, da_solicitud_compra.id_solicitante, da_solicitud_compra.correlativo FROM `da_solicitud_compra`
...
/*
function get_solicitud_compra($solicitud){
  $pdo = Database::connect();
  $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  $sql = "SELECT *  FROM da_solicitud_compra  WHERE id_solicitud_compra=? ";

  $p = $pdo->prepare($sql);
  $p->execute(array($solicitud));
  $solicitud1 = $p->fetchAll();
  Database::disconnect();
