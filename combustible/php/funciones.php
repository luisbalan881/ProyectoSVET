...
$pdo = Database::connect();
$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
$sql = "SELECT T1.year,T1.mes,LPAD(T1.solicitud_id, 4,'0') AS IDX,
...
function get_carro_por_solicitud_by_id($year,$mes,$solicitud,$vehiculo,$dep_id){
  $pdo = Database::connect();
  $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  $sql = "SELECT T2.km_ini,T1.vehiculo_id,T1.nombre,T1.linea,T1.placa,T1.modelo,T1.c_c,T1.tipo,
          T1.cilindraje,T1.combustible_id,T1.color,T1.status,T1.user_id, T1.vice_id, T1.capacidad, T1.status_uso, T1.comision_id,
...
// agregar esto  a para tomar kilometrajes de la bitacora ingresado por los pilostos   get_carro_por_solicitud_by_bitacora

function get_carro_por_solicitud_by_bitacora($vehiculo){
  $pdo = Database::connect();
  $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  $sql = "SELECT MAX(km_final) as maxKm_final from `vp_bitacora_vehiculo` where vehiculo_id=?";
  $q = $pdo->prepare($sql);
  $q->execute(array($vehiculo));
  $vehiculos = $q->fetch(PDO::FETCH_ASSOC);
  Database::disconnect();
...
function get_carro_por_solicitud_by_solicitud($vehiculo){
  $pdo = Database::connect();
  $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
 // $sql = "select max(t1.km_fin) as km2 from vp_solicitud_cupon_vehiculo as t1 join vp_vehiculo as t2 where t2.vehiculo_id=? and t1.vehiculo_id=t2.vehiculo_id";
  $sql = "SELECT km_ini from `vp_solicitud_cupon_vehiculo` where solicitud_id=?";

//SELECT km_ini from `vp_solicitud_cupon_vehiculo` where solicitud_id=40 and year=2022
  // SELECT max(vp_solicitud_cupon_vehiculo.km_fin) as km2 FROM vp_solicitud_cupon_vehiculo 
  //
  
  $q = $pdo->prepare($sql);
  $q->execute(array($vehiculo));
  $vehiculos2 = $q->fetch(PDO::FETCH_ASSOC);
  Database::disconnect();
