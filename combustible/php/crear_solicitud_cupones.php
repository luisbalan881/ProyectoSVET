...
$sql0 = "SELECT COUNT(*)as conteo FROM vp_solicitud_cupon WHERE dep_id=?";
$q0 = $pdo->prepare($sql0);
$q0->execute(array($persona['dep_id']));
$conteo = $q0->fetch(PDO::FETCH_ASSOC);

if ($conteo['conteo'] == 0) {
  $correlativo = 1;
} else {
  $sql1 = "SELECT MAX(solicitud_id) as si FROM vp_solicitud_cupon WHERE dep_id=?";
  $q1 = $pdo->prepare($sql1);
  $q1->execute(array($persona['dep_id']));
