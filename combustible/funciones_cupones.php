...
INNER JOIN vp_vehiculo
ON vp_bitacora_vehiculo.vehiculo_id=vp_vehiculo.vehiculo_id
where vp_bitacora_vehiculo.status = ? ORDER BY vp_bitacora_vehiculo.id_bitacora  DESC";
    $p = $pdo->prepare($sql);
    $p->execute(array($$status1));
    $nombramientos = $p->fetchAll();
    Database::disconnect();
    return $nombramientos;
}
...
function bitacora_info($id){
    $pdo = Database::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = "SELECT id_bitacora, Destino,motivo,km_inicial, km_final, status FROM vp_bitacora_vehiculo where id_bitacora = ? ORDER BY id_bitacora";
    $q = $pdo->prepare($sql);
    $q->execute(array($id));
