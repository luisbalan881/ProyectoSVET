...
function consultarActividad ($tipo_id){
    $pdo = Database::connect();
    $sql = "SELECT id_actividad, nombre, status FROM `tb_actividad` WHERE id_actividad = ?";
    $p = $pdo->prepare($sql);
    $p->execute(array($tipo_id));
    $archivoTipo = $p->fetch(PDO::FETCH_ASSOC);
    Database::disconnect();
...
function consultarProyecto ($tipo_id){
    $pdo = Database::connect();
    $sql = "SELECT id_proyecto, nombre, status FROM `tb_Proyecto` WHERE id_proyecto = ?";
    $p = $pdo->prepare($sql);
    $p->execute(array($tipo_id));
    $archivoTipo = $p->fetch(PDO::FETCH_ASSOC);
    Database::disconnect();
...
function seguimiento_nuevo(){
	
	 $user_id = $_SESSION['user_id'];
	$motivo = $_POST['motivo'];//soli_fecha2, no_oficio
	$fecha=$_POST['soli_fecha2'];// esta atrapando fecha sin formato soli_fecha
$date1 = date('Y-m-d', strtotime($fecha));   // fecha inicio
    $desc = $_POST['soli_desc']; //tipo_denuncia
	$no = $_POST['no_oficio'];
	$usuario = $_POST['d_user'];
...
$date1 = date('Y-m-d', strtotime($fecha));   // fecha inicio	
	*/
	 ...
	 if ($_FILES[ "archivo"]) {
		 ...
		 $ruta = "archivos_oficios/".$nombre_final;
		 $subirarchivo = move_uploaded_file($_FILES["archivo"]["tmp_name"], $ruta);
		 if($subirarchivo){
			 
		 $pdo = Database::connect();
    $sql = "INSERT INTO da_oficio_control (fecha_finalizacion,usuario,no_oficio,motivo,url_oficio,estatus) values(?,?,?,?,?,?)";
    $q = $pdo->prepare($sql);
    $q->execute(array($date1,$usuario,$no,$motivo,$ruta,1));
    
    
    //correo
    
   $rta = mail('marvingarcia16@gmail.com', "mensaje :xd",$motivo);
    var_dump($rta);
    //64
	
	if($q){
		echo "<script>alert('se ha agredo su archivo');window.Location='index.php'</script>";
...
$date1 = date('Y-m-d', strtotime($fecha));   // fecha inicio	
	*/
	 ...
	 if ($_FILES[ "archivo"]) {
		 ...
		 $pdo = Database::connect();
    //$sql = "INSERT INTO da_oficio_control (fecha_finalizacion,usuario,no_oficio,motivo,url_oficio,estatus) values(?,?,?,?,?,?)"; //UPDATE  ds_oficios SET  estatus=? WHERE id_oficio=?
    
      $sql = "UPDATE da_oficio_control  SET observaciones=?, url_oficio_respuesta=?, estatus=? where id=?"; //UPDATE  ds_oficios SET  estatus=? WHERE id_oficio=?
    $q = $pdo->prepare($sql);
    $q->execute(array($motivo,$ruta,2,$no));
	...
	function seguimiento_nuevo_user($id2){
...
//$date1 = date('Y-m-d', strtotime($fecha));   // fecha inicio
//$id=$id2;
    $observaciones = $_POST['observaciones']; //tipo_denuncia
	$no = $_POST['no_oficio'];
	$usuario = $_POST['d_user'];
...
$date1 = date('Y-m-d', strtotime($fecha));   // fecha inicio	
	*/
	 ...
	 if ($_FILES[ "archivo"]) {
		 ...
		 $pdo = Database::connect();
    $sql = "INSERT INTO da_oficio_control (fecha_respuesta,observaciones,url_oficio,estatus) values(?,?,?,?)";   // cambiar a actualizar
    $q = $pdo->prepare($sql);
    $q->execute(array($date1,$observaciones,$motivo,2));
...
// ver todos los oficios de user

function oficios($user_id){
    $pdo = Database::connect();
    $sql = " SELECT da_oficio_control.id, da_oficio_control.fecha_creacion ,da_oficio_control.no_oficio,da_oficio_control.usuario,da_oficio_control.fecha_finalizacion,da_oficio_control.motivo,da_oficio_control.observaciones,da_oficio_control.url_oficio,da_oficio_control.url_oficio_respuesta,da_oficio_control.url_oficio_corregido,da_oficio_control.estatus, CONCAT(vp_user.user_nm1,' ',vp_user.user_ap1) as user1
FROM da_oficio_control
INNER JOIN vp_user
ON da_oficio_control.usuario = vp_user.user_id  where da_oficio_control.usuario = ? ORDER BY da_oficio_control.id  DESC";
    $p = $pdo->prepare($sql);
...
function actividad_nueva(){
    if (usuarioPrivilegiado()->hasPrivilege('crearSolicitudTransporte')){
        ...
        if($_FILES['arch_original']['name']){
          ...
          subir_archivo($_FILES['arch_original'],$arch_original,$allowed,$ext,$dirname);
      /*  if($_FILES['arch_original']['name']){
            //cabiar nombre del archivo que se va guardar
            $ext = strtolower(pathinfo($_FILES['arch_original']['name'],PATHINFO_EXTENSION));
            $allowed = array('csv','txt','pub','xls','xlsx','doc','docx','ppt','pptx','pdf','jpeg','png');
            ...
            $arch_original = sanear_string($arch_original);
                    ...
                    $arch_recibido = '';   nombre
                }  */
                $arch_user = $user->persona['user_id'];
                $arch_depto = $user->persona['dep_id'];
                $arch_tipo = $_POST['tipo_id'];
                $fechai = fecha_ymd($_POST['fechainicio']); 
                $fechaf = fecha_ymd($_POST['fechafin']); 
                $dep = $_POST['d_departamentos'];
                $proyectos = $_POST['proyecto_id'];
                $nombre = $_POST['nombre'];
                $hora = $_POST['hora'];
                $descr = $_POST['desc'];
               ...
               // $decripcion = $_POST['decripcion'];
                ...
                $pdo = Database::connect();
                $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $sql =  "INSERT INTO  tb_actividad (nombre,descripcion,lugar,fecha_inicio,fecha_fin,hora,material,autoridad,id_usuario,id_proyecto, id_departamento,personal,archivo,status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                $q = $pdo->prepare($sql);
                $q->execute(array($nombre,$descr,$lugar,$fechai,$fechaf,$hora,$materialr,$participa,$user_id,$proyectos,$dep, $personal,$arch_original,$status));
      ...
      /*  if($_FILES['arch_original']['name']){
            ...
            $arch_original = sanear_string($arch_original);
               ...
               // $decripcion = $_POST['decripcion'];
                ...
                $pdo = Database::connect();
                $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $sql =  "INSERT INTO  tb_actividad (nombre,descripcion,lugar,fecha_inicio,fecha_fin,hora,material,autoridad,id_usuario,id_proyecto, id_departamento,personal,status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
                $q = $pdo->prepare($sql);
                $q->execute(array($nombre,$descr,$lugar,$fechai,$fechaf,$hora,$materialr,$participa,$user_id,$proyectos,$dep, $personal,$status));
               // $arch_id = $pdo->lastInsertId();
                /*
                $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                foreach ($insti as $c) {
...
function archivos_completo(){
    $pdo = Database::connect();
    $sql = "SELECT tb_proyecto.nombre, vp_deptos.dep_nm ,tb_proyecto.descripcion, tb_proyecto.fecha,  tb_proyecto.status , tb_proyecto.fecha_fin  FROM `tb_proyecto` 
    join vp_deptos
    ON tb_proyecto.dep_id=vp_deptos.dep_id";
    $p = $pdo->prepare($sql);
    $p->execute();
    $archivos = $p->fetchAll();
    Database::disconnect();
...
function proyectos(){
    $pdo = Database::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = "SELECT tb_proyecto.id_proyecto, tb_proyecto.nombre, vp_deptos.dep_nm FROM tb_proyecto
    join vp_deptos
    on tb_proyecto.dep_id=vp_deptos.dep_id
    where tb_proyecto.status=1";
    $r = $pdo->prepare($sql);
    $r->execute();
    $proyectos = $r->fetchAll();
    Database::disconnect();
    return $proyectos;
}
...
function contro_of(){
    $pdo = Database::connect();
    $sql1 = "SELECT da_oficio_control.id ,da_oficio_control.fecha_creacion,da_oficio_control.no_oficio,da_oficio_control.usuario,da_oficio_control.fecha_finalizacion,da_oficio_control.fecha_respuesta,da_oficio_control.motivo, da_oficio_control.observaciones,da_oficio_control.url_oficio,da_oficio_control.url_oficio_respuesta,da_oficio_control.url_oficio_corregido,da_oficio_control.estatus, CONCAT(vp_user.user_nm1, vp_user.user_ap1 ) as user1  FROM `da_oficio_control`
join vp_user
on da_oficio_control.usuario=vp_user.user_id
ORDER BY da_oficio_control.id DESC";
    $p1 = $pdo->prepare($sql1);
    $p1->execute();
    $actividades = $p1->fetchAll();
    Database::disconnect();
    return $actividades;
}
...
function actividades(){
    $pdo = Database::connect();
    $sql1 = "SELECT tb_actividad.id_usuario ,tb_proyecto.id_proyecto,tb_actividad.id_actividad, tb_proyecto.id_proyecto, tb_proyecto.nombre,tb_actividad.hora,tb_actividad.material,tb_actividad.autoridad, tb_actividad.nombre as nombreactividad,tb_actividad.status,tb_actividad.lugar, tb_actividad.fecha_inicio,tb_actividad.personal,tb_actividad.archivo,tb_actividad.fecha_fin,tb_actividad.descripcion, tb_departamentos.nombre as departamento, vp_deptos.dep_nm, tb_actividad.fecha_creacion 
    FROM `tb_actividad`
    join tb_proyecto on tb_actividad.id_proyecto=tb_proyecto.id_proyecto 
    join tb_departamentos on tb_actividad.id_departamento=tb_departamentos.id_departamento
    join vp_deptos on tb_proyecto.dep_id=vp_deptos.dep_id  ORDER BY tb_actividad.id_actividad DESC";
    $p1 = $pdo->prepare($sql1);
    $p1->execute();
    $actividades = $p1->fetchAll();
    Database::disconnect();
...
function infProyectos(){
    $pdo = Database::connect();
    $sql1 = "SELECT tb_proyecto.id_user, tb_proyecto.id_proyecto,  tb_proyecto.nombre,tb_proyecto.descripcion,tb_proyecto.fecha, tb_proyecto.fecha_inicio, tb_proyecto.fecha_fin, tb_proyecto.status, vp_deptos.dep,  vp_deptos.dep_nm FROM `tb_proyecto`
    JOIN vp_deptos
    ON tb_proyecto.dep_id=vp_deptos.dep_id ORDER BY tb_proyecto.id_proyecto DESC ";
    $p1 = $pdo->prepare($sql1);
    $p1->execute();
    $infoProyecto = $p1->fetchAll();
    Database::disconnect();
    return $infoProyecto;
}


function cambiarStadoEnProceso ($tipo_id){
    $tipo_nombre = $_POST['tipo_nombre'];
    $tipo_status = $_POST['tipo_status'];
    $user_id = $_SESSION['user_id'];
    $status = 2;
    $pdo = Database::connect();
    $sql = "UPDATE tb_actividad SET status = ?
            WHERE id_actividad = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($status,$tipo_id));
    Database::disconnect();
    header("Location: index.php?ref=_52");
}


function cambiarStadoProyecto ($tipo_id){
    $tipo_nombre = $_POST['tipo_nombre'];
    $tipo_status = $_POST['tipo_status'];
    $user_id = $_SESSION['user_id'];
    $status = 2;
    $pdo = Database::connect();
    $sql = "UPDATE tb_proyecto SET status = ?
            WHERE id_proyecto = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($status,$tipo_id));
    Database::disconnect();
...
function cambiarStadoCancelado ($tipo_id){
    $tipo_nombre = $_POST['tipo_nombre'];
    $tipo_status = $_POST['tipo_status'];
    $user_id = $_SESSION['user_id'];
    $status = 0;
...
function cambiarStadoEnFinalizado ($tipo_id){
    $tipo_nombre = $_POST['tipo_nombre'];
    $tipo_status = $_POST['tipo_status'];
    $user_id = $_SESSION['user_id'];
    $status = 3;
...
function departamentosgt(){
    $pdo = Database::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = "SELECT * FROM tb_departamentos order by nombre ASC";
    $r = $pdo->prepare($sql);
    $r->execute();
    $departamentosgt = $r->fetchAll();
    Database::disconnect();
    return $departamentosgt;
