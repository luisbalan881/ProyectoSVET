function solicitar_cupones(year, mes, solicitud_id, dep_id) {
  $.ajax({
    ...
    success: function (data) {
      ...
      doc.setFontSize(12);
      doc.writeText(30, 118, 'DATOS DEL VEHICULO:', {
        align: 'left',
        width: 190,
      });
      ...
      for (var i = 0; i < data.data.length; i++) {
        ...
        doc.setTextColor(68, 68, 68);
        doc.setFontSize(12);
        doc.roundedRect(25, 59, 165, 43, 1, 1);
        //doc.writeText(0, 48 ,'Departamento Solicitante: '+ data.data[i].departamento,{align:'right',width:190});
        doc.writeText(0, 45, 'INFORME DE CALCULO DE COMBUSTIBLE', {
          align: 'right',
          width: 190,
        });
        doc.writeText(0, 53, data.data[i].solicitud, {
          align: 'right',
          width: 190,
        });
        ...
        doc.writeText(
          30,
          58,
          'DATOS TOMADOS EN CUENTA PARA CALCULO DE COMBUSTIBLE:',
          { align: 'left', width: 190 },
        );
        doc.writeText(
          30,
          63,
          'Distancia en Km a lugar de destino:' + data.data[i].dis,
          { align: 'left', width: 190 },
        );
        doc.writeText(
          30,
          68,
          'KM Retorno a oficinas centrales:' + data.data[i].dis,
          { align: 'left', width: 190 },
        );
        doc.writeText(
          30,
          73,
          'Dias aproximados de recorrido:' + data.data[i].dias,
          { align: 'left', width: 190 },
        );
        doc.writeText(
          30,
          78,
          'Recorrido interno perimetro del destino:' + data.data[i].km_interno,
          { align: 'left', width: 190 },
        );
        doc.writeText(30, 83, 'Precio por galon:' + data.data[i].p_galon, {
          align: 'left',
          width: 190,
        });
        doc.writeText(
          30,
          88,
          'Rendimiento del Vehiculo:' + data.data[i].rendimiento,
          { align: 'left', width: 190 },
        );
        doc.writeText(
          30,
          93,
          'Total en quetzales: Q' + data.data[i].res_calculo,
          { align: 'left', width: 190 },
        );
        doc.writeText(0, 93, '__Redondeado a : Q' + data.data[i].monto, {
          align: 'center',
          width: 190,
        });
        doc.writeText(30, 98, 'Destino:' + data.data[i].destino, {
          align: 'left',
          width: 190,
        });
        ...
        doc.writeText(28, p1, data.data[i].vehiculo, {
          align: 'center',
          width: 55,
        });
        doc.writeText(42, p1, data.data[i].linea, {
          align: 'center',
          width: 60,
        });

        doc.writeText(88, p1, data.data[i].rendimiento, {
          align: 'center',
          width: 10,
        });
        doc.writeText(112, p1, data.data[i].combustible, {
          align: 'center',
          width: 10,
        });
        doc.writeText(132, p1, data.data[i].entregado_a, {
          align: 'center',
          width: 30,
        });
        doc.text(174, p1, 'Q');
        doc.writeText(187, p1, data.data[i].monto, {
          align: 'right',
          width: 0,
        });
      ...
      doc.writeText(
        0,
        258,
        'Reporte Generado vicesis - Módulo control de cupones',
      ...
      doc.writeText(
        0,
        10,
        'Fecha y hora en la que se creó la solicitud: ' + f_d_solicitud,
