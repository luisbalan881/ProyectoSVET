...
function factura_nueva(){
    ...
    if (facturas_temporales() == 0){
        ...
        $q = $pdo->prepare($sql);
        $q->execute(array($fac_serie,$fac_num,$fac_fecha,$prov_id,$orden_id,$fac_1h,$fac_control,$fac_obs,$fac_descuento,$fac_status,$user_id,$fac_rev));
        $id = $pdo->lastInsertId();
        Database::disconnect();
    }
    return $id;
...
function requisicion_info($id){
    $pdo = Database::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = 'SELECT T1.req_id as req_id,req_num,req_status,req_fecha,req_obs,T1.req_user,T2.user_pref,T2.user_nm1,T2.user_ap1,T2.user_nm2,T2.user_ap2, T1.dep_id, dep_nm, coalesce(total.prod_total,0) AS prod_total, total.req_total AS req_total, T11.user_nm1 AS encargado_nm1, T11.user_ap1 AS encargado_ap1, egr_fecha
            FROM alm_requisicion AS T1
            left join vp_user AS T2 on T2.user_id = T1.req_user
