...
function renglones2(){
    $pdo = Database::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = "SELECT k.kx_num, c.nombre  from alm_kardex k
    INNER JOIN vp_catalogo_insumos c
    on k.prod_id = c.prod_id ";
...
function permiso_salida($user_id){
    $pdo = Database::connect();
    $sql = " SELECT * FROM rrhh_solicitud 
where id_user_solicita=? ORDER BY id_solicitud DESC";
//where id_user_solicita=? and id_tipo=0 or id_tipo=2 ORDER BY id_solicitud DESC";
    $p = $pdo->prepare($sql);
    $p->execute(array($user_id));
    $solicitud_permiso = $p->fetchAll();
    Database::disconnect();
...
/*
function permiso_salida2($user_id){
    $pdo = Database::connect();
    $sql = "SELECT rrhh_solicitud.id_solicitud,rrhh_solicitud.fecha_creacion, rrhh_solicitud.justificacion, rrhh_solicitud.fecha_inicio,  rrhh_solicitud.status,  rrhh_solicitud.dias_solicitado, rrhh_detalle_solicitud.hora_inicio, rrhh_detalle_solicitud.hora_fin, rrhh_detalle_solicitud.tipo_tiempo, rrhh_detalle_solicitud.dia_completo FROM rrhh_solicitud 
join rrhh_detalle_solicitud
ON
rrhh_solicitud.id_solicitud=rrhh_detalle_solicitud.id_solicitud
where id_user_solicita=? and id_tipo=2 ORDER BY rrhh_solicitud.id_solicitud DESC";//
    $p = $pdo->prepare($sql);
    $p->execute(array($user_id));
    $solicitud_permiso = $p->fetchAll();
    Database::disconnect();
...
function permiso_salida2($user_id){
    $pdo = Database::connect();
    $sql = "SELECT * FROM rrhh_solicitud_salida where id_usuario=? ORDER BY rrhh_solicitud_salida.id_solicitud_salida DESC";//
    $p = $pdo->prepare($sql);
    $p->execute(array($user_id));
    $solicitud_permiso = $p->fetchAll();
    Database::disconnect();
...
function permiso_salida3($user_id){
	$s=1;
    $pdo = Database::connect();
    $sql = "SELECT rrhh_solicitud_salida.status, rrhh_solicitud_salida.id_solicitud_salida, rrhh_solicitud_salida.fecha_solicitud,CONCAT(vp_user.user_nm1, ' ', vp_user.user_ap1) AS user, rrhh_solicitud_salida.justificacion, rrhh_solicitud_salida.fecha, rrhh_solicitud_salida.hora_inicio,rrhh_solicitud_salida.Hora_fin, rrhh_solicitud_salida.duracion, rrhh_solicitud_salida.tipo_tiempo FROM rrhh_solicitud_salida 
join vp_user 
on rrhh_solicitud_salida.id_usuario=vp_user.user_id Order by rrhh_solicitud_salida.id_solicitud_salida DESC";//
    $p = $pdo->prepare($sql);
    $p->execute(array($s));
    $solicitud_permiso = $p->fetchAll();
    Database::disconnect();
...
function permiso_salida_admin($user_id){
    $pdo = Database::connect();
		
$sql = "SELECT rrhh_solicitud.id_solicitud,  CONCAT(vp_user.user_nm1, vp_user.user_nm2, vp_user.user_ap1, vp_user.user_ap2) AS usuario,  rrhh_solicitud.fecha_creacion,rrhh_solicitud.justificacion, rrhh_solicitud.fecha_inicio, rrhh_solicitud.fecha_final, rrhh_solicitud.dias_solicitado,rrhh_solicitud.status ,rrhh_periodo_usuario.año  FROM `rrhh_solicitud`
join vp_user
...
on rrhh_detalle_solicitud.id_periodo_user=rrhh_periodo_usuario.id_periodo_user where rrhh_solicitud.id_tipo=0 or rrhh_solicitud.id_tipo=2  ORDER BY rrhh_solicitud.id_solicitud DESC ";
...
function permiso_salida_admin0($id3){
	
	$pdo = Database::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = "SELECT rrhh_solicitud.id_user_solicita, rrhh_solicitud.id_periodo, rrhh_solicitud.id_solicitud,  CONCAT(vp_user.user_nm1, vp_user.user_nm2, vp_user.user_ap1, vp_user.user_ap2) AS usuario, rrhh_solicitud.dias_consumidos_user,rrhh_solicitud.pendientes, rrhh_solicitud.fecha_creacion,rrhh_solicitud.justificacion, rrhh_solicitud.fecha_inicio, rrhh_solicitud.fecha_final, rrhh_solicitud.fecha_retorno, rrhh_solicitud.dias_solicitado,rrhh_solicitud.status ,rrhh_periodo_usuario.año  FROM `rrhh_solicitud`
join vp_user
on rrhh_solicitud.id_user_solicita=vp_user.user_id
join rrhh_detalle_solicitud
on rrhh_solicitud.id_solicitud=rrhh_detalle_solicitud.id_solicitud
join rrhh_periodo_usuario
on rrhh_detalle_solicitud.id_periodo_user=rrhh_periodo_usuario.id_periodo_user where rrhh_solicitud.id_solicitud=?  ORDER BY rrhh_solicitud.id_solicitud DESC";
    $q = $pdo->prepare($sql);
    $q->execute(array($id3));
...
function permiso_salida_admin2($user_id){
    $pdo = Database::connect();
		
$sql = "SELECT rrhh_solicitud.id_solicitud,  CONCAT(vp_user.user_nm1, vp_user.user_nm2, vp_user.user_ap1, vp_user.user_ap2) AS usuario,  rrhh_solicitud.fecha_creacion,rrhh_solicitud.justificacion, rrhh_solicitud.fecha_inicio, rrhh_solicitud.fecha_final, rrhh_solicitud.dias_solicitado,rrhh_solicitud.status ,rrhh_periodo_usuario.año, rrhh_detalle_solicitud.hora_inicio, rrhh_detalle_solicitud.hora_fin FROM `rrhh_solicitud`
join vp_user
on rrhh_solicitud.id_user_solicita=vp_user.user_id
join rrhh_detalle_solicitud
on rrhh_solicitud.id_solicitud=rrhh_detalle_solicitud.id_solicitud
join rrhh_periodo_usuario
on rrhh_detalle_solicitud.id_periodo_user=rrhh_periodo_usuario.id_periodo_user where rrhh_solicitud.id_tipo=2  ORDER BY rrhh_solicitud.id_solicitud DESC ";
