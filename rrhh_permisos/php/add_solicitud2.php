...
//$use = $per_rol2['id_usuario']; // No dias a actual


$sql3= "SELECT dias_consumidos, id_usuario from `rrhh_periodo_usuario` where id_periodo_user=?";
$p3 = $pdo->prepare($sql3);
...
$user11 = $per_rol3['id_usuario']; // dias_cunsumidos_actuales
...
$sql5= "SELECT id_periodo_user , status
FROM rrhh_periodo_usuario
WHERE id_periodo_user > ? AND id_usuario=?
ORDER BY id_periodo_user ASC
LIMIT 1";
$p5 = $pdo->prepare($sql5);
$p5->execute(array($id_periodo, $user11));
...
/*
SELECT id_periodo_user , status
FROM rrhh_periodo_usuario
WHERE id_periodo_user > 5 AND id_usuario=992
ORDER BY id_periodo_user ASC
LIMIT 1
*/
