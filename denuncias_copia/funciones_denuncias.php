...
function denuncia_nueva(){
	...
	$denunciado = $_POST['d_user'];
	 ...
	 if ($_FILES[ "archivo"]) {
		 ...
		 $pdo = Database::connect();
    $sql = "INSERT INTO tb_denuncia (motivo,descripcion,tipo,id_denunciado, id, archivo) values(?,?,?,?,?,?)";
    $q = $pdo->prepare($sql);
    $q->execute(array($motivo,$desc,$tipo,$denunciado, $user_id, $ruta));
    ...
    $pdo = Database::connect();
    $sql = "INSERT INTO tb_denuncia (motivo,descripcion,tipo,id_denunciado, id) values(?,?,?,?,?)";
    $q = $pdo->prepare($sql);
    $q->execute(array($motivo,$desc,$tipo,$denunciado, $user_id));
    Database::disconnect();*/
  ...
  if ( $ _ FILES [ " archivo " ] ) {
     ...
     $ruta
                        ...
                        basename ( $ _FILES [ " archivo " ] [ " name " ] ) ;
                         date ( " m - d - y " ) . " - " . date ( " H - i - s " ) . " - " $ nombre_base ;
       }
               " archivos / " . $ nombre_final ;
     $ subirarchivo move_uploaded_file ( $ _ FILES [ " archivo " ] [ " tmp_name " ] , $ ruta ) ;
     if ( $ subirarchivo ) {
       $ insertar SQL " INSERT INTO informes ( nombre , apellidos , fecha , archivo ) VALUES ( ' $ nombre ' ,
       ' Sapellidos ' , ' $ fecha ' , ' $ ruta ' ) " ;
       $ resultado mysqli_query ( $ conexion , $ insertarSQL ) ;
       if ( $ resultado ) {
          echo " < script > alert ( ' Se ha enviado su informe ' ) ; window.location = ' index.html < / script > " ;
       } else {
          printf ( " Errormessage : % s \ n " , mysqli_error ( $ conexion ) ) ;
    }
  } else {
...
function denuncias(){
    $pdo = Database::connect();
    $sql1 = "SELECT tb_denuncia.id_denuncia, tb_denuncia.fecha, tb_denuncia.id_denunciado, tb_denuncia.motivo, tb_denuncia.descripcion, tb_denuncia.archivo, tb_denuncia.tipo, tb_denuncia.estatus, CONCAT(vp_user.user_nm1,vp_user.user_nm2, vp_user.user_ap1, vp_user.user_ap2 ) as user1  FROM `tb_denuncia`
join vp_user
on tb_denuncia.id_denunciado=vp_user.user_id
ORDER BY tb_denuncia.id_denuncia DESC";
    $p1 = $pdo->prepare($sql1);
    $p1->execute();
